pipeline {
    ...
    stages {
        ...
        stage('Build Docker Image') {
            ...
            steps {
                script {
                    app = docker.build("grebrt00/react-app")
                    app.inside {
        ...
        stage('Push Docker Image') {
         ...
         stage('DeployToStaging') {
            ...
            steps {
                    script {
                        sh "docker pull grebrt00/react-app:${env.BUILD_NUMBER}"
                        try {
                        ...
                        sh "docker run --restart always --name react-app -p 1233:80 -d grebrt00/react-app:${env.BUILD_NUMBER}"
                    }
            }
        }
